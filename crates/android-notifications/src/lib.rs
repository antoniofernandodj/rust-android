...
mod android_impl {
    ...
    pub fn create_channel(id: &str, name: &str, importance: Priority) {
        ...
        let _ = create_channel_inner(&mut env, &context, id, name, importance);
        let _ = env.exception_clear();
    }

    fn create_channel_inner<'local>(
        ...
        let nm = get_notification_manager(env, context)?;
        // NotificationChannel is API 26+; returns None silently on older devices.
        ...
        let ch = env
            .new_object(
                ...
                &[
                    ...
                    JValue::Int(priority_to_importance(importance)),
                ],
            )
            .ok()?;
        env.call_method(
    ...
    pub fn show(
        ...
        let Ok(mut env) = vm.attach_current_thread() else { return };
        let context = unsafe { JObject::from_raw(ctx.context().cast()) };
        let _ = show_inner(&mut env, &context, id, channel_id, title, body, progress, ongoing);
        let _ = env.exception_clear();
    }
    ...
    fn show_inner<'local>(
        ...
        // App icon from ApplicationInfo
        let app_info = env
            .call_method(context, "getApplicationInfo", "()Landroid/content/pm/ApplicationInfo;", &[])
            .ok()?.l().ok()?;
        let icon_id = env
            .get_field(&app_info, "icon", "I")
            .ok().and_then(|v| v.i().ok())
            .unwrap_or(0x0108_0020); // android.R.drawable.ic_dialog_info
        env.call_method(&builder, "setSmallIcon", "(I)Landroid/app/Notification$Builder;", &[JValue::Int(icon_id)]).ok()?;

        if let Some(t) = title {
            let s = env.new_string(t).ok()?;
            env.call_method(&builder, "setContentTitle", "(Ljava/lang/CharSequence;)Landroid/app/Notification$Builder;", &[JValue::Object(&s)]).ok()?;
        }
        if let Some(b) = body {
            let s = env.new_string(b).ok()?;
            env.call_method(&builder, "setContentText", "(Ljava/lang/CharSequence;)Landroid/app/Notification$Builder;", &[JValue::Object(&s)]).ok()?;
        }
        if let Some((cur, total)) = progress {
            env.call_method(&builder, "setProgress", "(IIZ)Landroid/app/Notification$Builder;",
                &[JValue::Int(total as i32), JValue::Int(cur as i32), JValue::Bool(0)]).ok()?;
        }
        if ongoing {
            env.call_method(&builder, "setOngoing", "(Z)Landroid/app/Notification$Builder;", &[JValue::Bool(1)]).ok()?;
        }

        let notif = env.call_method(&builder, "build", "()Landroid/app/Notification;", &[]).ok()?.l().ok()?;
        env.call_method(&nm, "notify", "(ILandroid/app/Notification;)V",
            &[JValue::Int(id as i32), JValue::Object(&notif)]).ok()?;
        Some(())
    }
    ...
    pub fn cancel(id: u32) {
        ...
        let Ok(mut env) = vm.attach_current_thread() else { return };
        let context = unsafe { JObject::from_raw(ctx.context().cast()) };
        let _ = cancel_inner(&mut env, &context, id);
        let _ = env.exception_clear();
    }

    fn cancel_inner<'local>(env: &mut jni::JNIEnv<'local>, context: &JObject<'local>, id: u32) -> Option<()> {
        let nm = get_notification_manager(env, context)?;
        env.call_method(&nm, "cancel", "(I)V", &[JValue::Int(id as i32)]).ok()?;
        Some(())
    }
}
